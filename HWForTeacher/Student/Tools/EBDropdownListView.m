...
static CGFloat const kArrowImgHeight= 18;
static CGFloat const kArrowImgWidth= 11;
...
- (void)setupFrame {
    ...
    _arrowImg.frame = CGRectMake(IS_IPAD?CGRectGetWidth(_textLabel.frame)-10:CGRectGetWidth(_textLabel.frame)-5, viewHeight / 2 - kArrowImgHeight / 2, kArrowImgWidth, kArrowImgHeight);
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(dropdownListView:didSelectedItem:)]) {
        [self.delegate dropdownListView:self didSelectedItem:item];
    }
}
...
- (UIImageView*)arrowImg {
    if (!_arrowImg) {
        ...
        _arrowImg.image = [UIImage imageNamed:@"time_choose"];
