...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return contentH+(IS_IPAD?120:100);
}
...
-(void)loadMessagesData{
    ...
    [[HttpRequest sharedInstance] postWithURLString:kRemoteMessagesAPI parameters:params success:^(id responseObject) {
        ...
        [ZYHelper sharedZYHelper].isUpdateMessage = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
...
-(BlankView *)blankView{
    if (!_blankView) {
        _blankView = [[BlankView alloc] initWithFrame:CGRectMake(0, 60, kScreenWidth, 200) img:@"blank_message" msg:@"暂无消息数据"];
    }
    return _blankView;
}
