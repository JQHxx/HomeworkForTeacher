...
-(void)textFieldDidChange:(UITextField *)textField{
    ...
    if (phoneHasInput&&codeHasInput&&agreementHasSelected) {
        ...
        [self.nextBtn setImage:[UIImage drawImageWithName:@"landing_nextstep_1" size:self.nextBtn.size] forState:UIControlStateNormal];
...
-(void)getVertifyCodeAction:(UIButton *)sender{
    ...
    [[HttpRequest sharedInstance] postWithURLString:kGetCodeSign parameters:@{@"mobile":self.phoneTextField.text} success:^(id responseObject) {
...
-(void)loginForNextStepAction:(UIButton *)sender{
    NSString *deviceId = [[APPInfoManager sharedAPPInfoManager] deviceIdentifier];
     NSDictionary *params = @{@"mobile":self.phoneTextField.text,@"code":self.codeTextFiled.text,@"platform":@"ios",@"deviceId":deviceId};
    [[HttpRequest sharedInstance] postWithURLString:kLoginAPI parameters:params success:^(id responseObject) {
        NSDictionary *data = [responseObject objectForKey:@"data"];
        BOOL isLogout = [[data valueForKey:@"status"] boolValue];
        if (isLogout) {
            dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [NSUserDefaultsInfos putKey:kUserId andValue:user.tid];
            [NSUserDefaultsInfos putKey:kUserToken andValue:user.token];
            [NSUserDefaultsInfos putKey:kLoginPhone andValue:self.phoneTextField.text];
            ...
            NSString *aliasStr=[NSString stringWithFormat:@"%@%@",tempStr,user.tid];
            ...
            if (!kIsEmptyString(user.subject)&&kIsArray(user.grade)&&user.grade.count>0) {
...
-(UILabel *)codeLab{
    if (!_codeLab) {
        _codeLab = [[UILabel alloc] initWithFrame:CGRectMake(40,self.phoneTextField.bottom+25, 160, IS_IPAD?30:20)];
...
-(UITextField *)codeTextFiled{
    if (!_codeTextFiled) {
        _codeTextFiled = [[UITextField alloc] initWithFrame:CGRectMake(40, self.codeLab.bottom+8,IS_IPAD?300:142, IS_IPAD?50:40)];
...
-(UIButton *)getCodeBtn{
    if (!_getCodeBtn) {
        _getCodeBtn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth-(IS_IPAD?180:120), self.codeLab.bottom+10,IS_IPAD?160:100,IS_IPAD?54:40)];
...
-(UIButton *)agreeBtn{
    if (!_agreeBtn) {
        ...
        _agreeBtn = [[UIButton alloc] initWithFrame:CGRectMake((kScreenWidth-btnW)/2.0, kScreenHeight-45,tempW+30,IS_IPAD?30:20)];
...
-(UIButton *)serviceBtn{
    if (!_serviceBtn) {
        NSString *str = @"《服务协议》";
        CGFloat btnW = [str boundingRectWithSize:CGSizeMake(kScreenWidth,IS_IPAD?30:20) withTextFont:[UIFont regularFontWithSize:12]].width;
        _serviceBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.agreeBtn.right, kScreenHeight-45, btnW,IS_IPAD?30:20)];
